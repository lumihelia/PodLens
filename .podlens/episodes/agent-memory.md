...
1. **全历史上下文的Prefill开销随历史累积而呈二次方级增长，且存在中段信息丢失风险**。
   - 锚点：1. Introduction · "prefill costs scale"
   - 类型：事实
2. **外部记忆系统通过解耦上下文长度与存储容量来克服长上下文处理的系统级限制**。
   - 锚点：1. Introduction · "decoupling capacity"
   - 类型：主张
3. **对于大语言模型介导的记忆系统，记忆构建所消耗的能量占据了智能体生命周期的绝大部分**。
   - 锚点：4.2. Construction Dominates the Agent Lifecycle · "exceeds total query-phase energy across 300"
   - 类型：事实
4. **智能体记忆构建本质上是一个以 Prefill 和 Embedding 为主导的重读取、轻写入的工作负载**。
   ...
   - 类型：事实
5. **在并行服务时，构建任务的巨大 Prefill 吞吐量会占用 KV 缓存并与低延迟的问答查询产生直接的资源争抢**。
